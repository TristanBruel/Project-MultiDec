...
#############################
# Galaxies considered
galaxy_names = c("NubeculaMinor","Sculptor","UrsaMinorDwarf", "DracoDwarf",
                 "NubeculaMajor")

# Sky positions
skyAngles = list(c(-72.8002,13.1583*12/180), c(-33.709,15.0389*12/180), c(67.2128,227.297*12/180),
                 c(57.9153,260.051*12/180),c(-69.7561,80.8941*12/180))

t = 1302220800

# Distances (in kpc)
distances = c(60.3, 78.2, 68.4, 79.1, 50)
...
for (gal in galaxy_names){
  ...
  for (dt in 1:time_nb){ # measurements over 3 days, 1 per half an hour
    ...
    ind1=1+(dt-1)*N
    ind2=N+(dt-1)*N
    print(sprintf("signal %s from galaxy %s @ distance: %f kpc @ time: +%s hours. Covpbb mean:%f. Covpbb median: %f",
                  signal_name, gal, dist, 0.5*(dt-1), mean(result[ind1:ind2,2]), median(result[ind1:ind2,2])))
  }
  
  filename=sprintf("results/extraGala/results_AA_%s_f2_%s_%s_multiDec_ET.txt", 
                   filtering_method, signal_name, gal)
