...
if(method=='singleDec'){
  ...
  for (dt in 1:time_nb){ # measurements over 2 days, 1 per hour
    
    t2 = t+(dt-1)*3600 # GPS time in seconds
    ...
    for (i in 1:N){
      ...
      r=specPdgrm(nopadd$y,nopadd$t,l=l,p=p,fs=fs,method='ar',
                  actPlot=FALSE,main=paste(signal_name,detectors[1]));
    ...
    ind1=1+(dt-1)*N
    ind2=N+(dt-1)*N
    print(sprintf("signal %s @ distance: %f kpc @ GPS time: %s. Covpbb mean:%f. Covpbb median: %f",
                  signal_name, dist, t2, mean(result[ind1:ind2,2]), median(result[ind1:ind2,2])))
    
    filename=sprintf("results/GalaCenter/results_AA_%s_f2_%s_%s.txt", 
                     filtering_method, signal_name, detectors[1])
    write.table(result, file=filename, sep=" ", row.names=FALSE, col.names=FALSE)
  }
  
}else if (method=='multiDec'){
  ...
  for (dt in 1:time_nb){ # measurements over 2 days, 1 per hour
    ...
    filename=sprintf("results/GalaCenter/results_AA_%s_f2_%s_multiDec2.txt", 
                     filtering_method, signal_name)
    write.table(result, file=filename, sep=" ", row.names=FALSE, col.names=FALSE)
  }
  
}else{warning("Method must be either 'singleDec' or 'multiDec'")}